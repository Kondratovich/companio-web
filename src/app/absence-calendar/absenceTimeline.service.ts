...
export class AbsenceTimelineService {
  ...
  getAbsenceTimelines(userId: string): Observable<AbsenceTimeline[]> {
    return this.http
      .get<AbsenceTimeline[]>(`${this.apiUrl}/absences?userId=${userId}`, { headers: this.headers })
      .pipe(catchError(this.handleError));
  }

  getAbsenceTimeline(id: string): Observable<AbsenceTimeline> {
    return this.http
      .get<AbsenceTimeline>(`${this.apiUrl}/absences/${id}`, { headers: this.headers })
      .pipe(catchError(this.handleError));
  }

  createAbsenceTimeline(absence: AbsenceTimeline): Observable<AbsenceTimeline> {
    return this.http
      .post<AbsenceTimeline>(`${this.apiUrl}/absences`, absence, { headers: this.headers })
      .pipe(catchError(this.handleError));
  }

  updateAbsenceTimeline(absence: AbsenceTimeline): Observable<AbsenceTimeline> {
    return this.http
      .put<AbsenceTimeline>(`${this.apiUrl}/absences/${absence.id}`, absence, { headers: this.headers })
      .pipe(catchError(this.handleError));
  }

  deleteAbsenceTimeline(id: string): Observable<AbsenceTimeline> {
    return this.http
      .delete<AbsenceTimeline>(`${this.apiUrl}/absences/${id}`, { headers: this.headers })
      .pipe(catchError(this.handleError));
  }
